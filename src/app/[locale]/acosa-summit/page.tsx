...
import { getSummitPage } from "@/lib/cms/fetch";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return { title: locale === "fr" ? "Sommet ACOSA" : "ACOSA Summit" };
}

export default async function SummitPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const summit = await getSummitPage(locale as AppLocale);
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-[var(--color-navy)]">
        <div className="absolute inset-0">
          <Image
            ...
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-navy)] via-[var(--color-navy)]/90 to-[var(--color-navy)]/60" />
        </div>
      ...
      <section className="bg-white py-14 sm:py-16 border-b border-black/5">
        <Container>
          <Reveal>
